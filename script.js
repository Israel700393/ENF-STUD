...
function safeJSONParse(key, defaultValue) {
    try {
        ...
        console.warn(`Erro ao carregar ${key}, usando valor padrão:`, e);
...
const state = {
    ...
    flashcards: safeJSONParse('flashcards', []),
    images: safeJSONParse('images', []),
    events: safeJSONParse('events', []),
    chatHistory: safeJSONParse('chatHistory', []),
    notes: safeJSONParse('notes', []),
    currentMonth: new Date().getMonth(),
    currentYear: new Date().getFullYear(),
    studyTime: parseInt(localStorage.getItem('studyTime')) || 0,
    pomodoroSessions: safeJSONParse('pomodoroSessions', { today: 0, total: 0, lastDate: new Date().toDateString() }),
    studyHistory: safeJSONParse('studyHistory', []),
    currentNoteId: null
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        console.log('1️⃣ Iniciando tema...');
        initTheme();
        
        console.log('2️⃣ Iniciando navegação...');
        initNavigation();
        
        console.log('3️⃣ Iniciando flashcards...');
        initFlashcards();
        
        console.log('4️⃣ Iniciando galeria...');
        initGallery();
        
        console.log('5️⃣ Iniciando calendário...');
        initCalendar();
        
        console.log('6️⃣ Iniciando chat...');
        initChat();
        
        console.log('7️⃣ Iniciando timer...');
        initTimer();
        
        console.log('8️⃣ Iniciando anotações...');
        initNotes();
        
        console.log('9️⃣ Atualizando estatísticas...');
        updateStats();
        
        console.log('🔟 Atualizando eventos...');
        updateUpcomingEvents();
        
        console.log('⏰ Iniciando timer de estudo...');
        startStudyTimer();
        
        console.log('✅ Aplicação iniciada com sucesso!');
    } catch (error) {
        console.error('❌ Erro ao iniciar aplicação:', error);
        alert('Erro ao carregar o site. Verifique o console (F12) para mais detalhes.');
    }
});
...
function initNavigation() {
    ...
    console.log(`Encontrados: ${navBtns.length} botões de navegação, ${sections.length} seções`);
    
    if (!sidebar) console.warn('⚠️ Sidebar não encontrado');
    if (!sidebarOverlay) console.warn('⚠️ SidebarOverlay não encontrado');
    if (!menuToggle) console.warn('⚠️ MenuToggle não encontrado');
    if (!closeSidebar) console.warn('⚠️ CloseSidebar não encontrado');
